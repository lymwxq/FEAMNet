...
def convbn(input, out_planes, kernel_size, stride, dilation, use_bias=False):
    seq = Conv2D(out_planes, kernel_size, stride, 'same', dilation_rate=dilation, use_bias=use_bias)(input)
...
def feature_extraction(sz_input, sz_input2):
    ...
    print(123, layer4.get_shape().as_list())

    branch11 = convbn(layer4, 4, 1, 1, 1)
    branch12 = convbn(branch11, 4, 3, 1, 1)

    branch21 = convbn(layer4, 6, 1, 1, 1)
    branch22 = convbn(branch21, 8, 3, 1, 3)

    branch31 = convbn(layer4, 8, 1, 1, 1)
    branch32 = convbn(branch31, 12, 3, 1, 5)
    ...
    print(234, branch12.get_shape().as_list(), branch22.get_shape().as_list(), branch32.get_shape().as_list())
    
    out = concatenate([branch12, branch22, branch32, layer2, layer4, branch4, branch3, branch2, branch1])


    lastconv = convbn(out, 16, 3, 1, 1)
    lastconv = Activation('relu')(lastconv)
    lastconv = Conv2D(4, 1, (1, 1), 'same', use_bias=False)(lastconv)
    print(567, lastconv.get_shape().as_list())
...
def define_FEAMNet(sz_input, sz_input2, view_n, learning_rate):
